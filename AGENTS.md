...
自动合并开启后,TASK 与 ADR 是这条流水线**唯一**的权威约束来源。ChatGPT
负责指挥与审查(读派发队列、建 Issue 触发 Codex、审查 PR),**需要新 TASK
或新 ADR 时由它提出需求、Claude Code 撰写**。让下达任务的人同时定义约束
自己的规则,会让「允许修改的文件」这类护栏失去意义——而那是目前防范围
蔓延最有效的一条。
