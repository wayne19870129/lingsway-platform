...
**ADR-030(2026-09-19)起:Claude Code 负责设计架构、撰写 ADR/TASK、
建 Issue 派活给 Codex、定期监督、必要时拉闸叫停;ChatGPT 的职责收窄为
对精确 head SHA 做独立审查。**需要新 TASK 或新 ADR 时仍由 Claude Code 撰写。

ADR-029 §7 那条理由(「让下达任务的人同时定义约束自己的规则,护栏就失去
意义」)在 ADR-030 下依然成立,只是指向不同:那条护栏(「允许修改的文件」)
约束的是**执行者 Codex**,而 Claude Code 不执行实现任务,所以由它同时写
TASK 并派活不触碰该护栏——被约束的一方仍然是另一个 agent。同时这样做
**消除了** ADR-029 自己记录的第 1 条反对意见(审查者审查自己的规格):
规格作者(Claude)与审查者(ChatGPT)现在是两个不同的 agent。
