...
8. **main 分支有机械保护(2026-09-19 实测确认,此前本条写反了)。**
   ...
   **关于自动合并(2026-09-18 修订,依据 ADR-029):**
   曾经存在过一个有条件自动合并的例外
   (`.github/workflows/claude-automerge.yml`,`TASK-T18`),被 User 撤销——
   实践中审查连续多轮给出新的、有效的 `NEEDS_CHANGES`,PR 长期卡在返工
   循环里,**从未真正走到过自动合并那一步**,只消耗审查/修复轮次。

   本条当时写着"不得在没有 User 重新明确要求的情况下重新引入任何形式的
   自动合并"。**User 已于 2026-09-18 重新明确要求**,并在 Claude 逐条列出
   风险后重申选择全开。因此自动合并由 **ADR-029** 重新引入,其完整条件、
   断路器与定时审计契约以该 ADR 为准。

   需要记住的差别:上一次的失效是「太保守、走不到合并」,这一次的风险是
   反过来的——**它可能真的合并,并且合并错的东西**。因此 ADR-029 同时
   引入了断路器(`.github/automerge-enabled`)与每 6 小时的独立架构审计;
   **这两者是自动合并成立的前提,不是可选装饰**。

   > **⚠️ 2026-09-19:上面这两个前提,现在只剩一个。**
   >
   > 断路器**仍在**(ADR-030 §5a 的停机/复机不对称一字未动)。
   > **独立架构审计已经没有了**——定时那部分由 ADR-033 取消,
   > 三个"强制人工触发"的节点由 **ADR-034** 取消,现在只有 User 开口才审。
   >
   > **按本条自己的措辞,这属于"削弱前提",本该先撤销 ADR-029。**
   > 没有那样做,依据是一个**可以被数出来的事实**:
   > **自动合并至今一个 PR 都没有合过**——#135 到 #153 全部由 User 人工
   > 合并,S07(#152)更是明确打了 `no-automerge`。
   > **ADR-029 要防的那个风险(机器合并了错的东西)至今没有发生过一次。**
   >
   > **这是一个有条件的接受,不是取消。** 条件写在 ADR-034 §6:
   > **一旦出现第一个由 workflow 自动合并、且 User 事前未看过的 PR**,
   > 这个依据即刻失效,必须二选一——**恢复独立审计,或者关掉自动合并**。
   > 在那之前,把 `.github/automerge-enabled` 保持为 `false`
   > 是最省事的等效做法。**不要在没有审计的情况下依赖自动合并。**

   仍然不变:**生产部署不自动化**,`deploy-*.yml` 的 `DRY_RUN` 闸门与人工
   批准不受影响。
...
**这三个 docs 目录的"Claude Code 独占"在 ADR-029 下尤其重要,不是形式。**
自动合并开启后,TASK 与 ADR 是这条流水线**唯一**的权威约束来源。
**ADR-034 之后这条只会更吃重**:审查频率降到"User 开口才有",
**规格质量因此成为质量的主要防线**——一份写错的 TASK,现在没有任何定期
检查会替你发现。

**ADR-032 + ADR-033 + ADR-034(均 2026-09-19)起:Claude Code 负责设计
架构、撰写 ADR/TASK、**仅在 User 开口时做审查**、必要时拉闸叫停;
**派活与审查都由 ChatGPT 承担**,执行者是 **Codex 桌面版**。
**需要新 TASK 或新 ADR 时仍由 Claude Code 撰写**——这一条是写权限,
不是审查,**不受 ADR-034 影响**。

> **2026-09-19 更正本段三处过期表述**(本次修改经 User 确认):
> - ~~「建 Issue 派活给 Codex」~~ —— **Issue 不再是派活载体**,
>   `codex-dispatch.md` 模板已删除(ADR-032 §6b);`@codex` 在 Issue 上
>   实测不触发任何东西。现在是 ChatGPT 写指令、**User 复制粘贴**给
>   Codex 桌面版(ADR-032 §6a)。
> - ~~「Claude Code ……派活」~~ —— **ADR-030 把派活移给 Claude 的那一条
>   已被 ADR-032 §3 撤销**,派活回到 ChatGPT。
> - ~~「定期监督」~~ —— **已被 ADR-033 撤销**,改为 **User 触发**;
>   **ADR-034 又取消了仅剩的三个"强制事件节点"**(接真实外部系统前、
>   生产部署前、工作线完成后),现在**只有 User 开口才审**。
>   **注意:取消的是"必须叫 Claude 审",不是"必须有人批准"**——
>   下面「禁止自主执行」整张表一个字没动。
>   健康摘要仍每 2 小时采集,但**没有固定读者**(代价见 ADR-033 §5)。

ADR-029 §7 那条理由(「让下达任务的人同时定义约束自己的规则,护栏就失去
意义」)依然成立,但**支点已经换了**(ADR-032 §3a):在 ADR-030 下它靠的是
"派活的人(Claude)不是被约束的人";ADR-032 把派活交回 ChatGPT 之后,
**它改为靠"派活的人不是写规格的人"**——

- **派活** = 把已写好的 TASK 交给执行者,谁做都行;
- **规格** = TASK 里的目标/约束/允许修改的文件/验收标准,**这才是审查的判据**。

ChatGPT 审查时对照的是**它没有写的那份 TASK**,所以 ADR-029 记录的第 1 条
反对意见(审查者审查自己的规格)仍然被消除。**因此本文件写权限表里
「`docs/82-tasks/` 与 `docs/80-decisions/` 由 Claude Code 独占」这一条,
是上述论证成立的唯一依据,不得随之放松。**

(`docs/85-agent-operating-model.md` 的角色表曾写成 ChatGPT"写 TASK 文件",
与本表冲突;那是 2026-09-18 引入的错误,已按本表和 ADR-029 §7 更正。)
...
- **User(人)**:提出业务需求,验收实际效果,决定是否合并 PR、是否上线。
  ...
  见「铁律」第 8 条。)
- **ChatGPT Work**(ADR-030 起职责收窄):对 PR 具体某个 commit SHA 做独立
  审查。**不再负责派活、也不再负责整理需求与验收标准**——需求与验收标准
  由 Claude Code 写进 TASK 文件。Work 目前是**按事件触发的审查者**,不是持续运行的
