...
8. **main 分支有机械保护(2026-09-19 实测确认,此前本条写反了)。**
   ...
   **这两者是自动合并成立的前提,不是可选装饰**。任何削弱它们的改动都要
   先撤销 ADR-029。
...
**ADR-032 + ADR-033(均 2026-09-19)起:Claude Code 负责设计架构、
撰写 ADR/TASK、**User 下令时做审查**、必要时拉闸叫停;**派活与审查都由
ChatGPT 承担**,执行者是 **Codex 桌面版**。**需要新 TASK 或新 ADR 时仍由
Claude Code 撰写。
