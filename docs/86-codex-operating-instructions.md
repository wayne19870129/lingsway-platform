...
> 这份文件是给 **Codex Cloud（网页版，连 GitHub）** 的常驻项目指令。
> User 把它设进 Codex 的「自定义指令」，或在每个任务开头指向它。
>
> **你用的是 5.6 Luna（轻推理），审查你的 ChatGPT 用的是 5.6 Soar（高推理）。**
> 这是刻意的分工（ADR-031）：**判断在它那边，执行在你这边。** 你的价值是
> 把明确的指令准确、完整、可验证地落地——不是替它重新想一遍。
>
