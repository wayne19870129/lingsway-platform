...
| **Codex Cloud 代码审查** | 自动审查（保留） | 按 PR 自动给改进建议 | 任何判定权——它的 verdict 闸门不认 |
...
> 刚推上去还没合并的东西看不到（见 §1）。
>
> 最后更新：2026-09-19（ADR-031：思考/执行分离，返工上限 5→3）。
...
| 任务 | 状态 |
|---|---|
| **S11** 补风险分类器路径表 | PR #144 进行中。首次派发用的 Codex Cloud，已弃用（ADR-032）；缺的 checkout 由 Claude 补上 |
| **S07** 订单队列计费 | 可以派给 **Codex 桌面版**。**这个 PR 要打 `no-automerge`，由 User 人工合**——理由见下 |

> **2026-09-19（ADR-032）执行者变更**：Codex **Cloud** 已弃用（无法选择模型
> 与推理档位、额度不可控、沙箱推不了分支）。执行者是 **Codex 桌面版**，
> 派活由你交给 User 粘过去。**Cloud 的代码审查仍然开着**，会在 PR 上自动留
> 建议——有用，但**不构成合并条件**（它的 `author_association` 是 `NONE`，
> 闸门不认它的 verdict，已实测）。
