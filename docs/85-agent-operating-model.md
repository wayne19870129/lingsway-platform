...
**你用的是 5.6 Soar 高推理、额度近乎无限；Codex 用的是 5.6 Luna 轻推理、
额度紧张。所以判断全部由你做，Codex 只照着改。**（ADR-031 §1–2）

Codex 那边的轻推理**不是偏好，是约束**——它要真的读仓库、改文件、跑测试、
推分支，这些本身就耗额度，额度有限时只有轻推理档位跑得完一个完整任务。
所以**别指望它能替你发现你漏掉的东西**。你漏了，就是漏了。
...
| 任务 | 状态 |
|---|---|
| **S11** 补风险分类器路径表 | TASK 已就绪，**这是新流水线的第一个真实任务**，刻意让它走完整自动合并流程 |
| **S07** 订单队列计费 | 等 Codex 额度重置（2026-09-20 08:09）后再派。**这个 PR 要打 `no-automerge`，由 User 人工合**——理由见下 |
...
当前已用到：**ADR-031**、**TASK-S11**。下一个分别是 **ADR-032**、**TASK-S12**。
