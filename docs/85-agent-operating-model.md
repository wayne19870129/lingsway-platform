...
>
> **ChatGPT 读不到这个仓库**，所以本文件对它而言是**由 User 整份粘贴**的，
> 不是"让它自己去读"。每次实质性更新本文件后，User 需要重新粘一次；
> §5 的状态快照尤其容易过期，正确做法见 §1（让 Codex 回报真实状态）。
>
...
## 1. 你读不到仓库——所以必须先让 Codex 取状态

**前提（2026-09-18 更正）：ChatGPT 网页版没有仓库访问权限，读不到本文件
以及任何 `.md`。Codex 有。** 本节原先写成"你每次开工前必须读这几个文件"，
那是错的——你根本读不了。

因此你的状态来源只有三个，按可信度排序：

1. **让 Codex 去读并回报**（最可信，见下方模板）；
2. User 粘给你的文件内容；
3. 本文件 §5 的快照（**会过期**，只能当背景，不能当事实）。

**硬性纪律：** 你**不得**凭记忆或推测断言仓库里某个文件写了什么、某个
函数是什么样。凡是要基于仓库事实做判断的，先让 Codex 取回来。说错一次，
后面所有指令都会建立在错的前提上。

### 开工前的状态刷新指令（复制给 Codex）

```text
只读任务，不要改任何文件、不要建分支、不要提 PR。

请依次读取并原样回报以下内容：
1. git log --oneline -5，以及当前 main 的 HEAD SHA
2. 开着的 PR 列表（编号、标题、head SHA、CI 结论）
3. docs/83-project-continuity.md 全文
4. AGENTS.md 的「铁律」「模块写权限」「流程」三节
5. ls docs/82-tasks/ 与 ls docs/80-decisions/
6. <如果本次任务已知涉及某区域：把相关的 ADR / TASK 文件全文也贴出来>
...
拿到回报之后再决定做什么。**`docs/83-project-continuity.md` 是最重要的
一份**——当前进度、provider 接线矩阵、未决缺陷清单（§8）、spec-vs-reality
漂移表（§9）都在里面。
...
> ⚠️ 本节是写下它那一刻的事实，**不会自动更新**。ChatGPT 读不到仓库，
> 所以它手上这份永远是 User 上次粘贴时的版本。把它当**背景**，不要当
> **依据**——要基于事实做判断时，按 §1 让 Codex 回报真实状态。
