...
闸门会自己合。**别为了"有产出"在 PASS 后面附一堆建议**，那会让 Codex 又动起来，
把一个已经可以合的 PR 推成新 SHA，你的 PASS 反而作废。
...
**所以要如实知道一件事**：ADR-029 自动合并开启后，`main` 之后
**在 User 下令之前没有独立检查**。代价与理由写在 ADR-033 §3 与 §5——
简单说，当前每个 PR 都由 User 人工合并，**User 本人就是那道关口**。
**这也意味着 §6.1 第三层熔断（连续自动合并 3 个就停下汇报）现在是把人
拉回环里的主要机制，不是可以顺手跳过的一条。**

Claude 审查时发现 Critical 仍会**直接拉断路器**（把
`.github/automerge-enabled` 置为 `false`）。**如果你发现自动合并停了，
先去看最近的审计/停机 PR，不要急着把开关打开——复机必须由 User 人工合并，
这是刻意的不对称（ADR-030 §5a，未改动）。**
...
> 1. **PR 打 `no-automerge`、由 User 人工合并**——这条流水线至今**没有
>    一个 PR 是 workflow 自动合并的**，而 ADR-034 §2 的安全论证正建立在
>    "每个 PR 都由 User 把关"这个可以被数出来的事实上。**不要主动去改它。**
> 2. **派活前先做范围闭包检查**——见下。
...
> **ADR-033 之后第三条是承重的，不是保险。** 定期监督取消了，**它现在是把
> 人重新拉回环里的主要机制**。平台不强制它（跟返工轮次上限一样，靠你遵守），
> 所以**跳过它没有任何东西会报错**——这正是它必须被当真的原因。
> 任何放宽它的提议，都要先重新评估 ADR-033。
