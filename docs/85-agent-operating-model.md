...
> ~~**你仍然不写 TASK / ADR**~~ —— **2026-09-20（ADR-036）：ADR-032 当时
> 「ChatGPT 不写 TASK」这条限制已被撤销。现在的规则是：**
> **你写 TASK（`docs/82-tasks/**`），但仍然不写 ADR（`docs/80-decisions/**`
> 与 `docs/81-reviews/**` 仍是 Claude 独占）。** ADR-032 §3a 用「TASK 由
> Claude 独占」来论证审查独立性，那个前提已不成立——代价与剩下的防线见
> ADR-036 §4 与 §0 顶部的 ADR-036 框。
>
...
>
> 最近一次整体审阅：2026-09-20（Claude Code，**ADR-036 角色调整：取消每个
> 执行阶段回 Claude 确认的固定卡点**）。**当前角色版本以 ADR-036 为准，
> 不是 ADR-030。**
...
| **ChatGPT（你）** | **大脑 / 指挥 / 规格** | 读 §5 队列选任务、**写 TASK 文件**、维护 §5 队列、拆解执行阶段、把任务交给 Codex 桌面版、审查 PR、写指令卡、**处理返工**、**一个阶段完成后直接进入计划中的下一阶段** | **写 ADR**、改代码、合并 |
| **Codex 桌面版** | 执行者 | 按 TASK 实现、跑测试、建分支、提 PR、按 review 指令返工 | 决定做什么、**自行扩大架构范围**、自行合并 |
| **Claude Code** | 架构 | 设计**整体架构**与**总体计划**、**撰写 ADR**、**User 主动触发时**做阶段性审核 / 调整、必要时**拉闸叫停** | 日常实现；实时盯过程；**写 TASK**（ADR-036 已下放）；**维护 §5 队列**（同上）；**定期审查**；**任何"到点就必须审"或"每个阶段必须回来确认"的卡点**（ADR-033 / 034 / 036 全部取消）；合并 |
...
一句话（**2026-09-20 按 ADR-036 更新**）：**你定规格、指挥并审查，
Codex 桌面版执行，Claude 只写 ADR、且只在 User 主动触发时做阶段性审核；
所有 PR 由 User 人工合并。**

> ## ⚠️ 2026-09-20（ADR-036）：每阶段回 Claude 的固定卡点已取消
>
> User 的决定：**「取消每个执行阶段必须回 Claude 确认的固定卡点。」**
> Claude 负责整体架构、总体计划、ADR，以及 **User 不定时主动触发**的阶段性
> 审核 / 调整；**你根据已经接受的架构和计划持续推进日常执行**。
>
> | 事 | 之前 | 现在 |
> |---|---|---|
> | `docs/82-tasks/TASK-*.md` | Claude 独占 | **你写**（Claude 也能写，但不再是必经之路） |
> | "要不要先有 TASK 文件" | 门槛卡在 Claude | **你按 §2.1 自己判** |
> | §5 派发队列 | Claude 维护 | **你维护** |
> | **一个执行阶段做完** | 回来找 Claude 确认架构 / 计划 | **不用。直接进入已有计划中的下一阶段** |
> | 返工 | —— | **你自己处理**（§3 的轮次规则仍适用） |
> | `docs/80-decisions/ADR-*.md` | Claude 独占 | **Claude 独占，没变** |
> | 发现必须改一条已接受 ADR | 找 Claude | **停止自行推进 → 报告 User → 是否叫 Claude 由 User 决定** |
>
> **不要把最后一行读成新卡点。** 它替换掉的是"ChatGPT → Claude → 继续"
> 这个往返；`AGENTS.md` 铁律 5（改架构前必须先有 ADR）本来就在，本次没动，
> 也动不了。**旧的"每个 checkpoint 自动找 Claude"不恢复。**
>
> **代价是真的，写在这里而不是藏着**：你现在写规格、派活、又审查，
> **"规格写错了"这类缺陷失去了独立的发现者**。历史上这类缺陷出现过三次
> （S07 两次漏「允许修改的文件」、S11 漏 `risk-classify.yml` 的 checkout
> 前提），**三次都不是审查抓住的**——两次 CI，一次执行者按规则停住上报。
> 完整论证与剩下的几道防线见 ADR-036 §4。**所以：写 TASK 时的范围闭包检查
> 比以前更重要**（四条轴各走一遍，见 §5.0）。
...
> ⚠️ ~~**你派活，但你不写规格——这个区分是整套安全论证的支点
> （ADR-032 §3a）。**~~ **2026-09-20 由 ADR-036 撤销。保留原文，因为它
> 说的代价是真的，只是 User 明确选择付这个代价。**
>
> **原论证（仍然有效的部分）**：ADR-029 记录过一条反对意见——「ChatGPT 将
> 审查自己规格下产出的代码，`main` 之前再无独立检查。」当时的回答是：
> 不会，因为 **TASK 由 Claude 独占撰写**，所以你审查时对照的是**一份你
> 没有写的 TASK**；独立性保在规格这一层，不在派发这一层。
>
> **现在这个支点没有了。** ADR-036 把 TASK 下放给你，**那条反对意见因此
> 成立了**：你写规格、你派活、你审查。
>
> **不要因为这段话还在就以为你不能写 TASK——你能。** 留着它是为了
> **你在写 TASK 时知道现在没人替你兜底**：
>
> - 范围闭包检查要**沿四条轴各走一遍**（函数/协议、数据库列、API 契约、
>   渲染字段），见 §5.0 的教训框；
> - 「允许修改的文件」写死，不写 `backend/app/**` 这种笼统路径；
> - 验收标准要能被机械验证，别写"确保正确"。
>
> **剩下的几道防线**（ADR-036 §4）：实现者仍然是另一个 agent；ADR 仍是
> Claude 独占，你无法自我授权改架构；**每个 PR 都由 User 亲手合并**；
> 9 个必需状态检查；User 不定时叫 Claude 做阶段性审核。
...
~~**TASK 文件由 Claude Code 写，不是你写。**~~
**2026-09-20（ADR-036）：TASK 文件现在由你写。** 缺 TASK 就自己在
`docs/82-tasks/` 建一个，按 §2.2 的四段写满；**不用等任何人**。
需求与验收标准仍然只能存在于 TASK 文件里，这一条没变。
...
- **自动返工上限 3 轮**（ADR-031 §6，2026-09-19 从 5 轮收紧）。第 4 轮仍有
  ...
  **停下来交给 User**，不要继续猜。
  ...
  > 正确动作是**停下来把 TASK 改对，重新派一次**——
  > **2026-09-20（ADR-036）：改 TASK 现在是你自己的事，不用找 Claude。**
  > 只有"这次返工暴露的是某条已接受 ADR 的结论必须改"才停下来报告 User。
...
>
> **2026-09-20（ADR-036）：上面那句"缺 TASK/ADR 时仍然必须叫"已经只剩
> 一半成立。** TASK 的独占写权限下放给你了，**只有 ADR 还是 Claude 独占**。
>
> **而且连 ADR 这一条，路由也变了：你不再"回头找 Claude"，你去找 User，
> 由 User 决定要不要叫醒 Claude。** 见 ADR-036 §2。

| 触发条件 | 怎么办 | 是否强制 |
|---|---|---|
| **需要新 TASK 文件** | **你自己写**（ADR-036 把 `docs/82-tasks/` 下放给你了） | 不叫 Claude |
| 任务怎么切、粒度多细、先做哪个、允许改哪些文件、某个实现细节怎么选、要不要补测试、返工怎么处理 | **你自己定** | 不叫 Claude |
| **一个执行阶段完成了** | **直接进入已有计划中的下一阶段** | 不叫 Claude |
| **必须改变某条已接受 ADR 的结论** | **停止自行推进 → 报告 User**，说清楚是哪条 ADR 的哪个结论、为什么必须改；**是否叫 Claude 由 User 决定** | 强制（铁律 5），但**路由到 User** |
| 要动 `domain/` 或 `providers/base.py` 的**架构决策** | 同上——交给 User | 同上 |
| ~~接线任何真实外部系统前~~ | ~~接线前安全边界复核~~ | ~~强制~~ **ADR-034 取消** |
| ~~任何生产部署前~~ | ~~部署前复核~~ | ~~强制~~ **ADR-034 取消** |
| ~~一条工作线完成后~~ | ~~全仓库审计~~ | ~~强制~~ **ADR-034 取消** |
| 你和 Codex 架构分歧，或同一问题反复返工 | 先自己判；实在定不了交给 User | 建议 |
| 感觉"文档说的和代码做的对不上了" | **文档过期你自己改**（TASK / `docs/85` / `docs/83` 都可以）；只有 ADR 要走上面那条 | 建议 |

> **"架构缺口"的判据，别读大了**（ADR-036 §2）：只有**必须推翻一条已接受
> ADR 的结论**才算。"ADR 没直接写这一条，但能从它唯一推出来"**不算**——
> 那种情况你自己定，并把结论写进 TASK。
...
Claude 的产出形态是：ADR、`docs/81-reviews/REVIEW-*.md`、以及直接修正过期的
记忆文件。**它不合并 PR，也不替 User 做决定。**
（**2026-09-20 起 TASK 文件不再是 Claude 的产出形态**——ADR-036 把它给你了。）

---

## 5. 派发队列（**你维护，你据此挑选并派发任务**）

> **2026-09-20（ADR-036）：这一节现在是你的，不是 Claude 的。**
> 你落地一个新 TASK、或一个任务状态变了，**在同一个 PR 里顺手把本节改对**。
> 不要把队列写进聊天消息——那正是本仓库反复吃亏的两份真相源。
>
...
> 而 TASK 仍由 Claude 独占撰写。~~ **2026-09-20：这个论证的前提没有了
> （ADR-036 把 TASK 给了你）。代价与剩下的防线见 §0 顶部的 ADR-036 框。**
>
> **这一节不是快照，是要随手维护的。** 每当**你**落地一个新的 TASK、或一个
> 任务状态变了，就在**同一个 PR 里**更新本节。
>
> 唯一仍需你自己核对的是**未合并分支**：本节写的是 `main` 上的状态，
> 刚推上去还没合并的东西看不到（见 §1）。
>
> 最后更新：2026-09-22（**`ADR-038` 落地（PR #175）：B2-B2c 的 resolver 契约
> 唯一化，新增 3 条证明 production wiring 的测试；随后经 mypy 与全仓静态调用点
> 扫描两轮闭包，其 allowed files 走完 **4 → 8 → 9 → 11**，当前为 **11 个**，
> 而 master union 只新增 `test_mihomo_safe_reload.py` 一个文件；
> B2-B2c 当前在途 PR #176 返工中；
> `B2-B2.1`~`.6` 已全部合并，`.6` 为 PR #174**；
> 2026-09-20：**B2-B2 触发返工熔断，已再切分为 `B2-B2.1` ~ `B2-B2.6`
...
| **S04-B2-B2** 的 `.1` ~ `.6` | ✅ **全部已合并**，`.6`（preparation + production wiring）为 PR #174 |
| **S04-B2-B2c** render / finalization | **在途：PR #176 返工中。** 前置**全部已解除**（含 `ADR-038` ✅ PR #175）。scope closure 历史 **4 → 8 → 9 → 11**，**当前 exact allowed files = 11 个**；接口落点见 ADR-038 §2，逐条清单与窄授权见 §5.1 第 8 行。**master union 只新增了 `backend/tests/guards/test_mihomo_safe_reload.py` 一个文件** —— 两个 integration 文件（`test_db_adapters.py`、`test_mihomo_reconciliation_mysql.py`）原本就在并集里，只是额外获得一项窄授权：做 ADR-038 bundle 签名迁移 |
| **S04-B2-B2d / B2-B3 / B2-B4** | 按 §5.1 的完整顺序依次排（单向依赖，见 TASK） |
...
| 8 | **S04-B2-B2c** render / finalization | 同上 | **⬅ 当前在途：PR #176 返工中。** 前置**全部已解除**（B2-B2a ✅、`.1`~`.6` ✅、**`ADR-038` ✅ PR #175 已合并**）。交付：`dialer-proxy` 渲染 + 明文在 render/finalization 边界解析，接口落点由 **ADR-038 §2** 唯一裁定，并有 3 条落在 `test_mihomo_reconciliation.py` 的测试证明 production wiring 不是 fake。**scope closure 历史：4 → 8 → 9 → 11，当前 exact allowed files = 11 个。**第 9 个由**全量 mypy** 发现（`backend/tests/guards/test_mihomo_safe_reload.py`，`:107`/`:138`/`:139`）；第 10、11 个由**全仓静态调用点扫描**发现（`backend/tests/integration/test_db_adapters.py` `:706`/`:779`，`backend/tests/integration/test_mihomo_reconciliation_mysql.py` 的 `_run_worker()` `:127`）—— mypy 看不见它们，因为 `pyproject.toml` 把 `backend/tests/integration` exclude 了。**两个 integration 文件只有窄授权**：仅做 ADR-038 bundle 签名迁移，不得改 rollback / restore / unhealthy 断言，不得改并发语义与 apply-count，不得提前做 B2-B4 的新验收；B2-B4 对它们的完整 integration / concurrency 验收职责不变。**master union 只 +1**（第 9 个）—— 两个 integration 文件早已在并集里。**`dialer-proxy` 在 pinned `metacubex/mihomo:v1.19.27` 上的 candidate validation 已在 #176 实施期间真实通过（exit code 0）。** |
...
> **✅ 2026-09-22：B2-B2c 的闭包已由 User 裁定补齐（8 → 9 → 11）。**
>
> 全仓 `grep -rn "\.finalize(" --include=*.py backend ops` 查出，被
> `ADR-038` bundle 签名打破的**不止** `test_mihomo_safe_reload.py` 那 3 处：
>
> | 文件 | 调用点 | 归属 |
> |---|---|---|
> | `backend/tests/integration/test_db_adapters.py` | `:706`、`:779` 的 `provider.finalize(template, FakeControllerSecretResolver())` | **已划入 B2-B2c** |
> | `backend/tests/integration/test_mihomo_reconciliation_mysql.py` | `:127`（`_run_worker()` 内）向 `reconcile_mihomo_job()` 第 3 个位置参数传 `cast(ControllerSecretResolver, …)` | **已划入 B2-B2c** |
>
> **裁决理由**：`ADR-038` §2.2/§2.4 改的是公共函数签名，所以该 checkpoint
> 必须同步所有现存静态调用点。**不得让 B2-B2c 合并后 `main` 上留着已知的
> 旧签名调用，再等未来 B2-B4 去修。** 这是机械迁移，**不需要新 ADR**。
>
> **授权是窄的**：只做 bundle 签名迁移。不得改 `test_db_adapters.py` 两个测试
> 原有的 rollback / restore / unhealthy-after-reload 断言；不得改
> `test_mysql_two_workers_only_one_applies` 的并发语义与断言，也不得改
> worker / locking / apply-count 行为；当前 template 没有 egress credential
> requirements，egress resolver 只作合法 bundle 成员，不得引入新业务语义；
> 不得提前实施 B2-B4 的新增测试。**B2-B4 对这两个文件的完整验收职责不变。**
>
> **master union 只 +1**（`test_mihomo_safe_reload.py`）——这两个 integration
> 文件早已在并集里，所以第二次扩的是 **checkpoint scope，不是 master union**。
>
> **方法论（写死，下次照做）**：公共签名变更的闭包必须**同时**走三条 ——
> ① `mypy` / type checker；② **全仓静态调用点 grep**；③ **CI 实际执行路径**。
> **只靠 mypy 不够**：`pyproject.toml` 把 `backend/tests/integration`
> `exclude` 了，而 `test_db_adapters.py` 那两处没有 skip 守卫、CI 的
> `backend` job 跑整棵 `backend/tests`，第 ②③ 条才是这次抓出它们的原因。
...
当前已用到：**ADR-038**、**TASK-S11**。下一个分别是 **ADR-039**、**TASK-S12**。

> **2026-09-22 更新**：`ADR-038`（Mihomo finalization 的 resolver 契约）
> 已落地 —— 它扩展 ADR-037 §6，定义 egress credential 的
> plaintext + revision identity 由哪个接口承载。
> **下一个可用的 ADR 编号是 `ADR-039`。**（认领时按下面那条规则对着
> `main` **加上所有 open PR** 一起查；本次认领时实测 open PR 为 0。）
>
...
- ~~**Claude Code**：每次定时审计之后（每 6 小时……）。~~
  **已取消（ADR-033 + ADR-034 + ADR-036）。Claude 不做任何定期汇报，因为它
  不再定期运行**——它只在 User 主动触发时运行一次，汇报就是那一次的产出本身。
  **ADR-036 §5 明确禁止新增任何 Claude 侧的 Routine / schedule / 自动唤醒 /
  定期审查。**
