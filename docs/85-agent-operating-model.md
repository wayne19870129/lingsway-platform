...
| **Claude Code** | 架构 | 设计架构、**撰写 ADR 与 TASK**、**User 下令时做审查**、必要时**拉闸叫停** | 日常实现；实时盯过程；**定期审查（ADR-033 起不做）**；合并 |
...
### 4.0 审查由 User 触发（ADR-033，**2026-09-19 改**）

> **本节原文是「定期监督」——Claude 按节奏自行唤醒审查。ADR-033 撤销了
> 那一条。** 现在是：**User 觉得需要审查时下令，Claude 才审。**
> 你**不需要**等 Claude 的定期结论，也**不要**假设有人在背后定期看着。
...
> **ADR-033 之后这张表更重要了**：原文说它列的是"定时审计覆盖不到的"——
> **现在没有定时审计，所有 Claude 的介入都从这里来。**
>
> **但有一条界线不要搞错**：下面标「强制」的几项是 **ADR-029 §6 的事件
> 触发闸门**，不是定期监督，**没有**被 ADR-033 撤销——接真实外部系统前、
> 生产部署前、一条工作线做完后，**该叫还是要叫**。
> ADR-033 取消的只是"没人叫也会自己来"那一部分。
>
> 另外，**User 随时可以直接下令做一次审查**，不必等下表任何一行成立。
...
> 刚推上去还没合并的东西看不到（见 §1）。
>
> 最后更新：2026-09-19（**S07 在途 PR #152；第二次范围闭包补充已修，见 §5.0**；
...
| 任务 | 状态 |
|---|---|
| **S07** 订单队列计费（续费/加购） | **在途：PR #152**（`task/s07-order-queue-billing`），已打 `no-automerge`，**由 User 人工合**。**2026-09-19 第二次因范围闭包漏项暂停返工**——见下 |

> **2026-09-19 · PR #152 暂停原因（第二次范围闭包漏项，TASK 已修）**
>
> S07 按 ADR-027 把 `UsagePeriod.order_id` 实现为 **NOT NULL + FK + UNIQUE**
> ——**这是对的，不要改回去**。但既有集成测试
> `backend/tests/integration/test_gateway_route_binding_lock.py` 里有一处直接
> 构造 `UsagePeriod(...)` 的 fixture 没传 `order_id`，而该文件**不在 TASK 的
> 允许清单里**。MySQL 8.4 全量跑出 **`1 failed, 802 passed`**，
> 唯一失败就是它（`Column 'order_id' cannot be null`）。
>
> **这是规格的问题，不是实现的问题**：Codex 不能越界改它，也**不能**为了让
> CI 变绿把 `order_id` 改回 nullable（那会违反 ADR-027）。TASK 已补入该文件
> 并记录依据。**TASK 修正合并后，PR #152 原样继续，不要新开 PR。**
>
> **值得记的一点**：这个文件**恰恰是上一轮闭包检查里被点名"查过、不加"的**。
> 当时的理由（没有 `OrderWorkflowState` 假实现）没错，但**只查了 Protocol
> 一条轴，没查 schema 那条轴**。做闭包检查要分别沿**函数/协议、数据库列、
> API 契约、渲染字段**各走一遍——只沿一条轴查出来的是"这条轴上闭合"。
>
> S07 仍是在途任务，仍 `no-automerge`、由 User 人工合并，
> **S07 → S04-B2-B 的顺序不变。**

> **2026-09-19 派发阻塞已解除（规格闭包修正）。**
> 第一次派 S07 时，Codex 桌面版**在开工前就停住并上报**：TASK 目标第 10 条
> 点名要改 `/subscriptions/{subscription_id}/renew`，而它所在的
> `backend/app/api/subscription.py` **不在「允许修改的文件」里**。
> **执行者没有越界，是 TASK 自相矛盾。**
>
> 已借这次对十条目标做完整范围闭包检查，补入 4 个文件
> （`api/subscription.py`、`services.py`、`tests/unit/test_services_wiring.py`、
> `tests/integration/test_provisioning_phase_boundary.py`），并把
> `docs/82-tasks/TASK-S07-*.md` 自身**移出**执行者允许清单——
> `AGENTS.md` 写权限表规定 `docs/82-tasks/` 由 Claude 独占，
> 让执行者改 TASK 等于让被约束的人改约束（ADR-032 §3a）。
> 逐条依据写在 TASK 的「范围闭包修正」一节。
>
> **这件事本身值得记：护栏按设计生效了。** 执行者撞到边界时**停下来上报**，
> 而不是自己扩大范围、也不是硬着头皮做——这正是 ADR-031 §5 和
> `docs/86` §7 要的行为。**代价是一轮往返，收益是没有一行越界代码。**
>
> **给派活方的一条操作教训**：写指令卡前，先确认目标点名的每个文件都在
> 允许清单里。这是 `docs/85` §3.1b「前置条件」那条在**规格层**的版本——
> S11 那次是指令的前置不成立，这次是目标与清单不闭合，**同一类缺陷的两种
> 形态**。
...
当前已用到：**ADR-033**、**TASK-S11**。下一个分别是 **ADR-034**、**TASK-S12**。
...
Claude 审查发现 Critical 就拉断路器（ADR-029 §6）。
