...
| **ChatGPT（你）** | **大脑 / 指挥 / 规格** | 读 §5 队列选任务、**写 TASK 文件**、维护 §5 队列、把任务交给 Codex 桌面版、审查 PR、写指令卡 | **写 ADR**、改代码、合并 |
| **Codex 桌面版** | 执行者 | 按 TASK 实现、跑测试、建分支、提 PR、按指令卡返工 | 决定做什么、自行扩大范围、自行合并 |
| **Claude Code** | 架构 | 设计架构、**撰写 ADR**、**User 开口时才做审查**、必要时**拉闸叫停** | 日常实现；实时盯过程；**写 TASK**（ADR-036 已下放）；**维护 §5 队列**（同上）；**定期审查**；**任何"到点就必须审"的强制节点**（ADR-033 + ADR-034 全部取消）；合并 |
...
Codex 桌面版执行，Claude 只写 ADR、且只在 User 开口时审查；
所有 PR 由 User 人工合并。**

> ## ⚠️ 2026-09-20（ADR-036）：执行权全部下放给你了
>
> User 的原话：「**别设置卡点啦，把权利都下放给 ChatGPT。你（Claude）只负责
> 整体架构设计和阶段性审核，这个我会不定时让你执行。让 ChatGPT 能够完全自主
> 地根据你的架构和计划执行，而不是做一段任务之后，又得回来找你确认架构和
> 计划。**」
>
> **具体变了什么：**
>
...
> | "要不要先有 TASK 文件" | 门槛卡在 Claude 那里 | **你按 §2.1 自己判** |
> | §5 派发队列 | Claude 维护 | **你维护** |
> | 一段任务做完 | 回来找 Claude 确认架构/计划 | **不用。直接开下一段** |
> | `docs/80-decisions/ADR-*.md` | Claude 独占 | **Claude 独占，没变** |
> | 发现必须改一条已接受 ADR | 找 Claude | **找 User**，由 User 决定要不要叫醒 Claude |
>
...
- **自动返工上限 3 轮**（ADR-031 §6，2026-09-19 从 5 轮收紧）。第 4 轮仍有
  ...
  **停下来交给 User**，不要继续猜。
  ...
  > 只有"这次返工暴露的是某条已接受 ADR 的结论必须改"才交给 User。
...
| 任务怎么切、粒度多细、先做哪个、允许改哪些文件、某个实现细节怎么选、要不要补测试 | **你自己定** | 不叫 Claude |
| **必须改变某条已接受 ADR 的结论** | **交给 User**，说清楚是哪条 ADR 的哪个结论、为什么必须改 | 强制（铁律 5），但**路由到 User** |
...
> 刚推上去还没合并的东西看不到（见 §1）。
>
> 最后更新：2026-09-20（**ADR-036：执行权全部下放给 ChatGPT——TASK、队列、
> 粒度都归你；只有 ADR 仍是 Claude 独占，且架构缺口路由到 User 而不是
> Claude。ADR-035：Mihomo deployment 常量落点已裁定，S04-B2-B2 解除阻塞。
> B2-B1 已合并（#156），客户端配置指南已合并（#160）**；
...
> `ADR-036`（执行权下放给 ChatGPT）**都已落地**，预定状态解除。
...
- ~~**Claude Code**：每次定时审计之后（每 6 小时……）。~~
  **已取消（ADR-033 + ADR-034）。Claude 不做任何定期汇报，因为它不再定期
  运行**——它只在 User 开口时运行一次，汇报就是那一次的产出本身。
  2026-09-20 实测确认：没有任何 Routine 或定时 workflow 会唤醒 Claude
  （详见 ADR-036 §5）。
