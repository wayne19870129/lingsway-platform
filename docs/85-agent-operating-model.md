...
| **Claude Code** | 架构与监督 | 设计架构、**撰写 ADR 与 TASK**、定期监督、必要时**拉闸叫停** | 日常实现；实时盯过程；合并 |
...
> 说法，**已经不成立**，不要按那个前提规划。
> 仍然不需要电脑的：CI、自动合并、健康摘要、Codex Cloud 的代码审查、
> Claude 的监督。
...
> `.github/ISSUE_TEMPLATE/codex-dispatch.md` 那个 Issue 模板**留着但不再是
> 主路径**：实测 `@codex` 在 Issue 上不触发任何东西。
>
...
> 刚推上去还没合并的东西看不到（见 §1）。
>
> 最后更新：2026-09-19（S11 风险分类器路径表已进入实现 PR）。
> **S08 自动合并流水线已合并（PR #136，`53cf663`），
...
| 任务 | 状态 |
|---|---|
| **S07** 订单队列计费（续费/加购） | **下一个要派的任务**，派给 Codex 桌面版。**这个 PR 要打 `no-automerge`，由 User 人工合**——理由见下 |

> **2026-09-19（ADR-032）执行者变更**：Codex **Cloud** 已弃用（无法选择模型
> 与推理档位、额度不可控、沙箱推不了分支）。执行者是 **Codex 桌面版**，
> 派活由你写好指令、交给 User 粘过去。**Cloud 的代码审查也一并关闭**
> （ADR-032 §5）——它和 Codex 桌面版**共用同一个额度池**，额度紧张时它每跑
> 一次就少一次执行机会。它确实抓到过真缺陷，所以这是**用能力换额度**，
> 不是去掉没用的东西；额度不再是瓶颈时应当重新打开。
>
> **交接全靠人工复制粘贴**，中间没有自动化（ADR-032 §6a）。

**监督机制已跑通（2026-09-19 实测）**：健康摘要在 **`pipeline-health` 分支**
上（不在 `main`），自动合并网关的采集与判定也已在真实 PR 上验证。
`main` 有 ruleset 保护——见 `AGENTS.md` 铁律 8 的更正。

**派 S07 时必须带上这一条：**

**S07 不走自动合并，给它打 `no-automerge`，由 User 人工合。**
它多文件、碰账务 provider、含周期滚动、零行现成代码，而且内含一条客户
今天就能踩到的缺陷（见下）。**在这条流水线还没有一次完整跑通 happy path
之前，不要拿它当第一个自动合并的样本。**

（`no-automerge` 标签 User 已于 2026-09-19 建好，可直接用。）
...
全部合并），所以这条排序规则现在只剩后半句：

**生产功能里 S07 优先于 S04-B2-B**，因为它含一条**客户今天就能踩到**的
缺陷（见下方速览末尾）。
...
当前已用到：**ADR-032**、**TASK-S11**。下一个分别是 **ADR-033**、**TASK-S12**。
