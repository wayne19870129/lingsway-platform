...
- **`dedup` job 必须在 `claude` job 之前跑,并且按
  `(PR 编号, PR 当前 head SHA)` 这个键去重**(Issue #50 明确要求"忽略
  ... 重复 delivery 和已经处理过的同一指令",提示词层面的建议不算满足
  这条,必须是 `claude` job 的 `if:` 依赖 `dedup` job 输出的一个可执行
  门禁)。这个门禁**只覆盖 PR 评论触发的返工场景**:同一个 PR、同一个
  head SHA 上的第二次(及以后)`@claude` 提及会被跳过,不会重复起一次
  会话;一旦有新 commit 推上去、head SHA 变了,门禁重新打开。**明确不
  覆盖 Issue 首次触发场景**:那种情况下还没有 PR/head SHA 可以作为去重
  键,目前仍然只靠提示词里"先查是否已有分支/PR"这条建议,加上 GitHub
  真实重复投递同一个 webhook 本来就很罕见——这是一个已知、如实披露的
  缺口,不是"已经解决"。
- 不自行合并本任务的 PR。
