...
PR #106 已人工合并到 `main`，ADR-020 已完成。PR #107 已人工合并到
`main` `5c67aba469f7beb82df13750247c90e2d1820e7a`，因此 full-config
composition slice 已完成，当前 frontier 转为 preservation/legal-deletion →
writer-guard/drift → existing-data reconciliation → Phase 2B final review：
...
该 composition slice 已完成；本阶段不包含 preservation/legal-deletion 之后的
writer-guard/drift、reconciliation、registry wiring、schema/migration、deploy
...
The remaining Compose v1/v2 portability contract is closed. The deployment resolver now runs backend-api's `Settings.from_env().accounting_provider` through `compose run -T --rm --no-deps backend-api`, emits one exact `__LINGSWAY_ACCOUNTING_PROVIDER__=...` sentinel, and rejects zero/multiple/noisy/invalid output or any probe failure. The same path works for Compose v2 and the supported `docker-compose` v1 fallback; Compose remains responsible for `LINGSWAY_ENV_FILE` and quoted dotenv parsing. The patched Marzban image verification remains before `compose up`, while the mock default remains the upstream image.
...
- 采用 commit-first：先 durable-commit desired B + identifier-only pending intent，再从 fresh committed B 全量 render/apply/verify，最后 durable-commit Mihomo projection finalization。只有该 projection finalization 可由 fresh read 证明、健康/readback/fingerprint/revision 全部匹配且 lock release 无 uncertainty 时才可标记 `MIHOMO_APPLIED`；这不改变外层 Order/Subscription/customer provisioning 状态，customer-facing APPLIED/success 仍须等第 6–9 步及既有 authoritative completion boundary 完成。
